...
test("test", async ({ page }) => {
  ...
  const firstPersonOption = page
    .locator(".oxd-autocomplete-dropdown > div")
    .first();
  const selectedPerson = await firstPersonOption.innerText();
  await firstPersonOption.click();
  ...
  await page
    ...
    .click();
  await page.getByText("Accommodation").click();
  await page.getByPlaceholder("yyyy-dd-mm").waitFor({ state: "visible" });
  ...
  await page
    .getByRole("listitem")
    .filter({ hasText: "Employee Claims" })
    .click();
